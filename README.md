...
**UPD: Репозиторий будет исправлен после завершения работы над ВКР.**
...
Решил поиграть с размерами свёрточных фильтров. Во-первых, решил их уравнять, чтобы получалось так, что в каждой ветки сети изучались разные по длине участки кардиограмм. Во-вторых, увеличил длину фильтров. Она достигла 60 (то есть больше 20% длины для самой короткой ветви), а точность на случайных участках выросла с 67% до 71%. Выросли, соотвественно, все остальные показатели (где-то до не самых честных, но 77-78% точности).

Осталось проверить идеи inception и resnet, насколько получится их применить для сигналов. Эксперименты с архитектурой (правда, до изменения длины фильтра) не привели ни к чему (добавление/удаление свёрточных и полносвязных слоёв). Качество либо оставалось таким же, либо падало.

**13.04.2017**

Проверил пользу многоканальности сети (а зачем, собственно нужно было всё это). Пришлось, правда, использовать выходы не последнего слоя сети. Поэтому сравнивались 450 признаков с предпоследнего слоя с 150 признаками с предпоследнего слоя, полученных от одной ветви. 

|          | All   | Only 150 |
| -------- | ----- | -------- |
| AUC      | 0.719 | 0.655    |
| Accuracy | 0.666 | 0.615    |

То есть другие ветви действительно что-то дают. Хотя по-хорошему надо обучить новую сеть с одним каналом, а то вдруг другие каналы замедляют обучение.

**10.04.2017**

Попробовал сравнить внутренние состояния. Запускал t-SNE и PCA.

1. Если взять и для каждого человека усреднить внутренние состояния после обработки сетью, то можно получиться следующую картинку:![1](images/1.png)

   Не то чтобы хорошо, но некую сгруппированность по классам увидеть можно.

2. Если взять по 2 человека из 2 классов и у каждого взять по 15 сигналов, по которым усреднить внутреннее состояние, то получится следующее:![2](images/2.png)
